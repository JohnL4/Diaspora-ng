...
****** New structure
       ...
       - clusters (metadata)
         - $clusterUuid
           ...
           - editors
           - viewers
           - ".read": /clusters/$clusterUuid/viewers.contains($uid)
           - ".write": /clusters/$clusterUuid/editors.contains($uid)
       - clusterData
         - $clusterUuid
           - xml
...
* Events
  ...
  #+BEGIN_SRC picture
    ...
    +-------------+              +---------------------+              
    | Available   |   <object>   | make a bunch        |              +-------+ \                            
    | System UIDs |------------->| of new observables, |- - - - - - ->| row 0 |  \
    +-------------+              | one per row         |              +-------+   \                          
                                 +---------------------+                           \               +----------+
                                            |                         +-------+     \              | LIST of  |
                                                                      | row 1 |      --- merge --->| cluster  |
                                            ...
                                            |
                                                          +---------+  (sort)
                                            \ - - - - - ->| Subject |---------> gack, we need a real diagramming tool
                                                          +---------+
  #+END_SRC 
