...
/**
 * Validate numbers given an "allowed values specification" in the form "i;j;[k,m];(n,p);[,q);[r,]", which means the
 * following values are allowed:
 *
 * <ul>
 *    <li> i
 *    <li> j
 *    <li> All values in the range [k,m] (inclusive) -- k must be less than or equal to m
...
export function allowedNumericValuesValidator( anAllowedValuesSpec: string): ValidatorFn
{
   ...
   return (control: AbstractControl): {[key: string]: any} => {
      ...
      if (isNaN( num))
         return {[SELECTOR]: {numberToBeValidated}}; // [SELECTOR]: ES6 computed property name
